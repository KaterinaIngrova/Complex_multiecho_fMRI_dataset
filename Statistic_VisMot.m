%% STATISTIC FOR SINGLE SUBJECT ANALYSIS - for task VisMot

function Statistic_VisMot
    ...
    for sub=1:nsub 
        ...
        for ses=1:nses
            ...
            P   = [];
            q   = spm_select('FPList',fcnidir,['^swtse_r.*' subjects{sub} '.*' sessions{ses,1} '.*_part-mag_bold.nii$']);
            P   = strvcat(P,q);
            tmpv=spm_vol(P);
            nscans = size(tmpv,1);
            for i=1:nscans
            ...
            for i=1:length(EInfo.TrialType)
                if contains(EInfo.TrialType{i},'active block')
                    EventType(i)=1; %Just active blocks are used for analysis        
                end
            end
            ...
            %Conditions                         
            s.condstruct(1) = struct(  'name',             'Visual motor task',...
            ...
            %Contrasts    
            nmr = 24; % number of regressors

            consessstruct{1}.tcon.name='Visual motor task positive';
            consessstruct{1}.tcon.convec=[1 zeros(1,nmr) 0];

            consessstruct{2}.tcon.name='Visual motor task negative';
            consessstruct{2}.tcon.convec=[-1 zeros(1,nmr) 0];
